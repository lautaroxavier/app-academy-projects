...
window.addEventListener('DOMContentLoaded', () => {
  ...
  for (let moleHead of moleHeads) {
    moleHead.addEventListener('click', event => {
      ...
      setTimeout(() => hideMole(event.target), 250);

      // UNCOMMENT THIS LINE OF CODE WHEN DIRECTED
      setTimeout(() => event.target.classList.remove('wgs__mole-head--whacked'), 200);

      setTimeout(() => event.target.classList.add('wgs__mole-head--hidden'), 250);
